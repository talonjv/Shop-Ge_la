...
function ProductItem({ product, navigate, setPreviewImage, genderParam }) {
  ...
  const hasSale =
    product.SalePercent && product.SalePercent > 0 && product.SalePrice && product.SalePrice > 0;

  // Điều hướng sang trang chi tiết sản phẩm
  // Tùy logic: nếu genderParam = "nam" => /do-nam/...,
  //            nếu genderParam = "nu"  => /do-nu/...,
  //            nếu không => /shirt/... (hoặc tuỳ bạn)
...
export default function ProductPageShirt() {
  const { genderParam } = useParams(); // Lấy param nam/nu (nếu có)
  const navigate = useNavigate();

  // Xác định selectedGender: nam => male, nu => female, mặc định => all
  ...
  useEffect(() => {
    axios
      .get("http://localhost:2000/api/v1/products-with-sale", {
        params: { categoryID: 1 }, // categoryID=1 => Áo
      })
      .then((response) => {
  ...
  // Đặt headingText
  let headingText = "Sản phẩm Áo";
  if (selectedGender === "male") headingText = "Áo Nam";
  if (selectedGender === "female") headingText = "Áo Nữ";
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row p-4">
        ...
        <div className="w-full md:w-4/5 p-4">
          ...
          {/* Tiêu đề dựa trên selectedGender */}
